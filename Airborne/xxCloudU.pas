...
procedure TCloud.Initialize(Width,Height:Integer);
var
  W,H  : Integer;
  Size : Integer;
begin
...
//  if ProjectorMask.Enabled then CreateObstaclesFromProjectorMask(Obstacles,W,H);
  ...
  SubtractedSize:=W*H*SizeOf(Single);
  GetMem(SubtractedData,SubtractedSize);
...
{  SmokePositionData[1]:=0;
  SmokePositionData[2]:=0;
  SmokePositionData[3]:=W;
  SmokePositionData[4]:=0;
  SmokePositionData[5]:=0;
...
// create a texture handle
  ...
  Case Components of
//    1: glTexImage2D(GL_TEXTURE_2D, 0, GL_R16F, width, height, 0, GL_RED, GL_HALF_FLOAT, nil);
//    2: glTexImage2D(GL_TEXTURE_2D, 0, GL_RG16F, width, height, 0, GL_RG, GL_HALF_FLOAT, nil);
//    3: glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB16F, width, height, 0, GL_RGB, GL_HALF_FLOAT, nil);
//    4: glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA16F, width, height, 0, GL_RGBA, GL_HALF_FLOAT, nil);
...
var
  ...
  Line    : PByteArray;
begin
  ObstacleBmp.Width:=GridWidth;
  ObstacleBmp.Height:=GridHeight;

  ClearBmp(ObstacleBmp,clWhite);
  ObstacleBmp.Canvas.Brush.Color:=clBlack;
  ObstacleBmp.Canvas.FillRect(Rect(0,0,100,100));
...
//  Result:=GridWidth-1-Round(CamX*(GridWidth/ImageW));
  Result:=GridWidth-1-Round(CamX*MaxImageW/TrackW*(GridWidth/ImageW));
...
// find the scaled coordinate of this chain pixel
      with Tracker.Chain[C].Link[L] do begin
        I:=Round((MaxImageH-1-Y)*SmokeTexture.W+X);
        Inc(DataPtr,I);
        DataPtr^.R:=255;
//        DataPtr^.G:=255;
//        DataPtr^.B:=255;
...
// hot pixels rise
  ...
  if Camera.MouseX>=0 then begin
    BlobPos.X:=ViewPortXToGridX(Camera.MouseX);
    BlobPos.Y:=ViewPortYToGridY(Camera.MouseY);
    Wd:=30; Ht:=60;

    ApplyOval(Temperature.Ping,BlobPos,Wd,Ht,Rz,ImpulseTemperature);
    ApplyOval(Density.Ping,BlobPos,Wd,Ht,Rz,ImpulseDensity);
  end;
...
//    SaveBmp;
  ...
  else if SmokeChars then glBindFrameBuffer(GL_FRAMEBUFFER,Density.Ping.FboHandle);

  ReadVelocityData

//  if RenderMode=rmVelocity then ReadVelocityData
//  else ReadDensityData;
end;


procedure TCloud.PreRender;
var
  GLColor : TGLColor;
begin
  VisualizeProgram.Active:=False;
