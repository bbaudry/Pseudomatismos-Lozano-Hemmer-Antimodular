...
uses
  ...
  Global, DShowUtils, FreeImage, FreeImageUtils, MatrixU, ImageU, Ipl, PvAPI,
...
type
  ...
  TProjectorLookUpTableEntry = record
    X,Y : Word;
  end;
  TProjectorLookUpTable =
    array[0..MaxImageW-1,0..MaxImageH-1] of TProjectorLookUpTableEntry;
...
type
  ...
  TCameraInfo = packed record
    ...
    Reserved    : array[1..255-16] of Byte;
  end;
...
// private vars
  ...
  public
    ...
    ProjTable   : TProjectorLookUpTable;
    MeasuredFPS : Single;
    ...
    procedure ShutDown;
    procedure CalculateProjectorLookUpTable;
...
function TCamera.AbleToGetDWordParameter(Param:String;var V:DWord):Boolean;
begin
  Result:=Assigned(HCamera) and
         (PvAttrUint32Get(HCamera,PChar(Param),V)=ePvErrSuccess);
end;

function TCamera.AbleToSetDWordParameter(Param:String;var V:DWord):Boolean;
begin
  Result:=Assigned(HCamera) and
         (PvAttrUint32Set(HCamera,PChar(Param),V)=ePvErrSuccess);
end;

function TCamera.AbleToGetEnumParameter(Param,Value:String):Boolean;
const
  MaxSize = 255;
var
  Size : DWord;
...
function TCamera.AbleToSetEnumParameter(Param,Value:String):Boolean;
begin
  Result:=False;
  if Assigned(HCamera) then begin
...
procedure TCamera.AddEnumsToStrings(Param:String;Strings:TStrings);
const
  MaxSize = 1024;
var
  RC,I,I1 : Integer;
...
procedure TCamera.SetExposureMode(Mode:String);
begin
...
var
  ...
  LineSize    : DWord;
  BufferSize  : DWord;
  Buffer      : array[0..MaxImageW*MaxImageH*3] of Byte;
  BufferPtr   : PByte;
  Y           : Integer;
  Line        : PByteArray;
begin
...
procedure TCamera.CalculateProjectorLookUpTable;
var
  X,Y    : Integer;
  ProjPt : TPixel;
begin
