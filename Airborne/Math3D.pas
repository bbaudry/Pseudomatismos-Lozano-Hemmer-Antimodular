...
function  AbleToFindTargetOnYPlane(var Location:TPositionAndOrientation;
                 Pan,Tilt,Height,ZOffset:Single;var Target:TPoint3D):Boolean;
function  AbleToFindTargetOnZPlane(var Location:TPositionAndOrientation;
               Pan,Tilt,Height,ZOffset:Single;var Target:TPoint3D):Boolean;
...
function  StageIntersectionPoint(Base,Tgt:TPoint3D):TPoint2D;

implementation

uses
  Dialogs, SysUtils, Math, QMatrix, MathUnit;
...
// with no pan or tilt, the vector will point straight down
  ...
  RxMatrix:=XRotationQMatrix(Tilt);
  RyMatrix:=YRotationQMatrix(Pan);

  V:=Point3DMultQMatrix(V,RxMatrix);
  V:=Point3DMultQMatrix(V,RyMatrix);

  LaserOffset:=Point3DMultQMatrix(LaserOffset,RxMatrix);
  LaserOffset:=Point3DMultQMatrix(LaserOffset,RyMatrix);

  RxMatrix:=XRotationQMatrix(+Location.Tilt);
  RyMatrix:=YRotationQMatrix(+Location.Pan);
  RzMatrix:=ZRotationQMatrix(-Location.Rotation);

  V:=Point3DMultQMatrix(V,RyMatrix);
  V:=Point3DMultQMatrix(V,RxMatrix);
  V:=Point3DMultQMatrix(V,RzMatrix);

// if V.Y=0, we're pointing parallel with the Y plane so there will be no Y=Height
// intersection point
  if V.Y=0 then Exit;

  LaserOffset:=Point3DMultQMatrix(LaserOffset,RyMatrix);
  LaserOffset:=Point3DMultQMatrix(LaserOffset,RxMatrix);
  LaserOffset:=Point3DMultQMatrix(LaserOffset,RzMatrix);

// recycle the LaserOffset var
  LaserOffset.X:=LaserOffset.X+Location.X;
  LaserOffset.Y:=LaserOffset.Y+Location.Y;
  LaserOffset.Z:=LaserOffset.Z+Location.Z;

// find the parametric T variable
  T:=(LaserOffset.Y-Height)/V.Y;

// find the intersection point
  Target.X:=LaserOffset.X-T*V.X;
  Target.Y:=Height;
  Target.Z:=LaserOffset.Z-T*V.Z;
  Result:=True;
end;
...
// apply the fixture p,t,r
  RxMatrix:=XRotationQMatrix(-Pose.Rx);
  RyMatrix:=YRotationQMatrix(-Pose.Ry);
  RzMatrix:=ZRotationQMatrix(+Pose.Rz);
  Target:=Point3DMultQMatrix(Target,RzMatrix);
  Target:=Point3DMultQMatrix(Target,RxMatrix);
  Target:=Point3DMultQMatrix(Target,RyMatrix);
...
var
  T : Single;
begin
  T:=Base.Z/(Base.Z-Tgt.Z);
  Result.X:=Base.X-T*(Base.X-Tgt.X);
  Result.Y:=Base.Y-T*(Base.Y-Tgt.Y);
end;

procedure AddPointToSum(var Pt,Sum:TPoint3D);
begin
  Sum.X:=Sum.X+Pt.X;
  Sum.Y:=Sum.Y+Pt.Y;
  Sum.Z:=Sum.Z+Pt.Z;
