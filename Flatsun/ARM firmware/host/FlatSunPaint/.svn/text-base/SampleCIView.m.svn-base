...
- (void)setImage:(CIImage *)image dirtyRect:(CGRect)r
{
    ...
    if (_fsInterface == 0) {
        _fsInterface = [[FSInterface alloc] init];
        [_fsInterface open];
    }
}
...
//    NSLog(@"drawRect");
    ...
    if ([NSGraphicsContext currentContextDrawingToScreen])
    {
        ...
        
        {
            ...
            unsigned char *pRGB = (unsigned char *) malloc (3 * width * height * 2);
            glReadPixels(x, y, width, height, GL_RGB, GL_UNSIGNED_BYTE, pRGB);
            
//            NSLog(@"drawRect %d %d %d %d", x, y, width, height);
            for (ix = 0; ix < width; ix ++) {
                for (iy = 0; iy < height; iy ++) {
                    int ip = (iy * width + ix) * 3;
                    unsigned char r = pRGB[ip + 2];
                    unsigned char g = pRGB[ip + 1];
                    unsigned char b = pRGB[ip + 0];
                    //                    NSLog(@"drawRect %d %d %d %d %d", ix, iy, (int) r, (int) g, (int) b);
                    [_fsInterface setPixelVal:0 atX:(x + ix)  atY: 255 - (y + iy) ];
#if 0
                    if (iy % 2 == 0) {
                        if (ix % 2 == 1) continue;
                    } else {
...
#endif
                    [_fsInterface setPixelVal:r atX:(x + ix)  atY: 255 - (y + iy) ];
                }
            }
            ...
            for (ix = 0; ix < 128; ix ++) {
                    ...
                    unsigned char g = pRGB[ip + 1];
                    unsigned char b = pRGB[ip + 0];
                    [_fsInterface setHaloVal:r at:ix];
            }
