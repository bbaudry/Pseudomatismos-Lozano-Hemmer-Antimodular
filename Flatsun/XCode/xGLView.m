...
/*rdColor1.colorDivider = 0.561;
  rdColor1.scale = 2.199;
  
  rdColor2.colorDivider = 1.0;
  rdColor2.scale = 2.017;
*/
...
// set up the projection matrix
  ...
  gluPerspective(fov, (float) w / (float) h, DIST_NEAR, DIST_FAR);
...
- (void) mouseDragged : (NSEvent *) event
{
  ...
  sceneRx = sceneRx + 180 * (mousePt.y - startPt.y) / self.bounds.size.height;
  sceneRz = sceneRz - 180 * (mousePt.x - startPt.x) / self.bounds.size.width;
...
#define RENDER_DELAY (10000) // 25 fps
