...
#define kCameraRz @"kCameraRz"
#define kCameraUseITable @"kCameraUseITable"
...
- (void) setToDefault
{
  ...
  self.gain = 20;
  self.shutter = 10;
  self.brightnessMode = 0;
  self.exposureMode = 0;
  self.gainMode = 0;
  self.shutterMode = 0;
  self.debayer = 1;
  self.flipTexture = 0;
  self.mirrorTexture = 0;
  self.rz = 0.0;
  self.useITable = NO;
}       
...
// go with defaults if our keys are missing
  if (![settings keyExists : kCameraBrightness]) {
    [self setToDefault];
//    [settings freeDictionary];
    return;
  }
  
  self.brightness = [settings intFromKey : kCameraBrightness];
  self.exposure = [settings intFromKey : kCameraExposure];
  self.gain = [settings intFromKey : kCameraGain];
  self.shutter = [settings intFromKey : kCameraShutter];

  self.brightnessMode = [settings intFromKey : kCameraBrightnessMode];
  self.exposureMode = [settings intFromKey : kCameraExposureMode];
  self.gainMode = [settings intFromKey : kCameraGainMode];
  self.shutterMode = [settings intFromKey:kCameraShutterMode];
  
  self.frameRateIndex = [settings intFromKey : kCameraFrameRateIndex];
  self.debayer = [settings intFromKey : kCameraDebayer];
  
  self.flipTexture = (BOOL) [settings intFromKey : kCameraFlipTexture];
  self.mirrorTexture = (BOOL) [settings intFromKey : kCameraMirrorTexture];
  
  self.rz = [settings floatFromKey : kCameraRz];
  self.useITable = (BOOL) [settings intFromKey : kCameraUseITable];
...
- (void) saveSettings
{
  ...
  [settings setInt : brightness forKey : kCameraBrightness];
  [settings setInt : exposure forKey : kCameraExposure];
  [settings setInt : gain forKey : kCameraGain];
  [settings setInt : shutter forKey : kCameraShutter];
  
  [settings setInt : brightnessMode forKey : kCameraBrightnessMode];
  [settings setInt : exposureMode forKey : kCameraExposureMode];
  [settings setInt : gainMode forKey : kCameraGainMode];
  [settings setInt : shutterMode forKey : kCameraShutterMode];
    
  [settings setInt : frameRateIndex forKey : kCameraFrameRateIndex];
  [settings setInt : (int) debayer forKey : kCameraDebayer];

  [settings setInt : (int) flipTexture forKey : kCameraFlipTexture];  
  [settings setInt : (int) mirrorTexture forKey : kCameraMirrorTexture];
  
  [settings setFloat : rz forKey : kCameraRz];
  [settings setInt : (int) useITable forKey : kCameraUseITable];
}
...
//[stopWatch start : 0];  
  ...
  if (debayer) { 
    ...
    if (useITable) {
      [iTable processData : data withWidth : imageW andHeight : imageH toDest : data];
    }
  }
...
//  [self loadSettings];
  ...
  [self showFeature : DC1394_FEATURE_SHUTTER];
...
{
  [self applyLock];
    [iTable initFromData : [self currentData] withWidth: imageW andHeight : imageH];
    [iTable save];
  [self removeLock];  
}
...
// take the first one
  ...
  NSLog(@"Using camera: %llu",list->ids[0].guid);
